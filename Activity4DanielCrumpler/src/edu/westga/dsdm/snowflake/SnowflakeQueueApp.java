 ...
 * @version Spring 2020
 */
public class SnowflakeQueueApp extends Frame {

	private LinkedListQueue<Line> lines = new LinkedListQueue<Line>();
	 ...
	 */
	public SnowflakeQueueApp() {
		this.lines = new LinkedListQueue<Line>();
	...
	public void run() {
		...
		this.lines.enqueue(new Line(pointA, pointB));
		this.lines.enqueue(new Line(pointB, pointC));
		this.lines.enqueue(new Line(pointC, pointA));

		// now make the snowflake
		while (!this.lines.isEmpty()) {
			Line line = this.lines.dequeue();
	...
	private void processLine(Line line) {
		...
		if ((dx * dx + dy * dy) < 10) {
			...
			this.lines.enqueue(new Line(line.start, pointA));
			this.lines.enqueue(new Line(pointA, pointB));
			this.lines.enqueue(new Line(pointB, pointC));
			this.lines.enqueue(new Line(pointC, line.stop));
		}
	}
